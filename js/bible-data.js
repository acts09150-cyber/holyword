/**
 * HolyWord - Bible Data (한국어 개역개정 기본)
 */
...
const DAILY_VERSES = [
  ...
  { text: '내가 확신하노니 사망이나 생명이나 어떤 피조물이라도 우리를 우리 주 그리스도 예수 안에 있는 하나님의 사랑에서 끊을 수 없으리라', ref: '로마서 8:38-39' },
...
const BOOK_EN_MAP = {
  ...
  '이사야':'isaiah','예레미야':'jeremiah','예레미야애가':'lamentations','에스겔':'ezekiel',
  ...
  '로마서':'romans','고린도전서':'1corinthians','고린도후서':'2corinthians','갈라디아서':'galatians',
  '에베소서':'ephesians','빌립보서':'philippians','골로새서':'colossians',
  '데살로니가전서':'1thessalonians','데살로니가후서':'2thessalonians','디모데전서':'1timothy',
  '디모데후서':'2timothy','디도서':'titus','빌레몬서':'philemon','히브리서':'hebrews',
  '야고보서':'james','베드로전서':'1peter','베드로후서':'2peter','요한일서':'1john',
...
async function fetchBibleChapter(bookName, chapter, lang = 'ko') {
  ...
  // 한국어 개역개정 로컬 데이터 우선
  if (lang === 'ko' && KRV_DATA[bookName] && KRV_DATA[bookName][chapter]) {
  ...
  try {
    ...
    const url = `${BIBLE_API_BASE}/${encodeURIComponent(bookEn)}+${chapter}?translation=${translation}`;
...
function getFallbackVerses(bookName, chapter) {
  if (KRV_DATA[bookName] && KRV_DATA[bookName][chapter]) {
    return KRV_DATA[bookName][chapter].map((text, i) => ({ number: i + 1, text }));
  }
  // API 연결 안될 때 안내 메시지
  return Array.from({ length: 5 }, (_, i) => ({
    number: i + 1,
    text: `${bookName} ${chapter}장 ${i+1}절 — 말씀을 불러오는 중입니다. 잠시 후 다시 시도해주세요.`
  }));
}
